...
C  or within a 'rectangular' window (pyramidal beam).
C
...
C           emitted with zero polarisation (SP1=SP2=SP3=0); however,
C           these photons may become partially polarised after being
C           scattered.
C             DEFAULT: none
C
...
C           (.LE. 200).
C             DEFAULT: EL=0.0, EU=EPMAX, NBE=100
C
...
C           respectively. Notice that this agrees with the usual meaning
C           of these terms only when primary particles impinge on the
C           system coming from below (i.e. with W>0).
C
C  >>>>>>>> Impact detectors.
C
...
C  parallelepiped (dose enclosure) whose edges are parallel to the axes
C  of the laboratory frame. The enclosure is defined by giving the
C  coordinates of its vertices. The dose is tallied using a uniform
C  orthogonal grid with NDBX, NDBY and NDBZ (.LE. 101) bins (= voxels)
C  along the directions of the respective coordinate axes. These numbers
C  should be odd, to make sure that the 'central' axes (i.e., lines that
C  join the centres of oposite faces of the enclosure) go through the
C  centres of a row of voxels. When an input value, say NDBX, is even,
C  it is replaced by NDBX+1.
C
...
C             directions, respectively. These values are assumed to be
C             odd and less or equal to 101.
...
C
      ...
      LOGICAL LSPEC,LGPOL,LPSF,LEXSRC,LEXBD,LSCONE,LINTF,LFORCE,
     1        LDILOG,LDELOG,LDOSEM
      CHARACTER*2 LIT
      CHARACTER*10 LC10A,LC10B
      CHARACTER*20 PMFILE,PFILE,PSFI,PSFDIO,SPCDIO,SPCDEO,PFILED,
     1             PFILER,SPCFSO
      CHARACTER*23 DATE23
...
C  ****  Geometry.
     ...
     1    KSURF(NB,NX),KFLAG(NB,NX),KALIAS(NS),KSLAST
      ...
      DIMENSION PARINP(20)
      DIMENSION DSMAX(NB),EABSB(3,NB)
C  ****  Extended source.
      DIMENSION IXSBOD(NB)
C  ****  Interaction forcing parameters.
      DIMENSION LFORCE(NB,3),WLOW(NB,3),WHIG(NB,3)
...
C  ----  Impact detectors (up to NIDM different detectors).
      ...
      DIMENSION PSFDIO(NIDM),SPCDIO(NIDM),LDILOG(NIDM)
      DIMENSION DIT(NIDM,NIDCM),DIT2(NIDM,NIDCM),DITP(NIDM,NIDCM),
     1  LDIT(NIDM,NIDCM),TDID(NIDM),TDID2(NIDM),DEDI(NIDM),IPSF(NIDM),
     1  IDCUT(NIDM)
      DIMENSION DIP(NIDM,NIDCM,3),DIP2(NIDM,NIDCM,3),DIPP(NIDM,NIDCM,3),
     1  LDIP(NIDM,NIDCM,3)
      DIMENSION EDIL(NIDM),EDIU(NIDM),BDIE(NIDM),NDICH(NIDM)
      DIMENSION EDILL(NIDM),EDIUL(NIDM),BDIEL(NIDM)
      DIMENSION KKDI(NIDM,3),RLAST(NIDM),RWRITE(NIDM)
C  ----  Energy distribution of fluence.
      DIMENSION SPCFSO(NIDM)
      DIMENSION FST(NIDM,NIDCM),FST2(NIDM,NIDCM),FSTP(NIDM,NIDCM),
     1  LFST(NIDM,NIDCM)
      DIMENSION FSP(NIDM,NIDCM,3),FSP2(NIDM,NIDCM,3),FSPP(NIDM,NIDCM,3),
     1  LFSP(NIDM,NIDCM,3)
C
C  ----  Energy-deposition detectors (up to NEDM different detectors).
      PARAMETER (NEDM=25,NEDCM=1000)
      DIMENSION SPCDEO(NEDM),LDELOG(NEDM)
      DIMENSION DET(NEDM,NEDCM),TDED(NEDM),TDED2(NEDM),DEDE(NEDM)
      DIMENSION EDEL(NEDM),EDEU(NEDM),BDEE(NEDM),NDECH(NEDM)
      DIMENSION EDELL(NEDM),EDEUL(NEDM),BDEEL(NEDM)
      DIMENSION KBDE(NB)
C
C  ----  Dose distribution (up to NDXM*NDYM*NDZM volume bins).
     ...
     1          DOSEP(NDXM,NDYM,NDZM),LDOSE(NDXM,NDYM,NDZM)
      DIMENSION DDOSE(NDZM),DDOSE2(NDZM),DDOSEP(NDZM),LDDOSE(NDZM)
...
C
   ...
   51   CONTINUE
      ...
      ENDIF
      EMIN=MAX(EMIN,0.0D0)
      EMAX=MAX(EMIN+50.0D0,MIN(EMAX,EPMAX))
...
C  ****  Bin sizes.
C
C  ----  The factor 1.000000001 serves to ensure that the upper limit of
C  the tallied interval is within the last channel (otherwise, the array
C  dimensions could be exceeded).
      BSE(1)=1.000000001D0*(EMAX-EMIN)/DBLE(NBE)
      BSE(2)=1.000000001D0*(EMAX-EMIN)/DBLE(NBE)
      BSE(3)=1.000000001D0*(EMAX-EMIN)/DBLE(NBE)
      BSTH=1.000000001D0*180.0D0/DBLE(NBTH)
      BSPH=1.000000001D0*360.0D0/DBLE(NBPH)
C
C  ************  Impact detectors.
C
      DO KD=1,NIDM
...
C
 ...
 1614     FORMAT(3X,'Energy window = (',1P,E12.5,',',E12.5,') eV',
     1      /3X,'Number of energy bins = ',I4,'  (logarithmic mesh)')
          ...
          BDIEL(NDIDEF)=1.000000001D0*(EDIUL(NDIDEF)-EDILL(NDIDEF))
     1      /DBLE(NDICH(NDIDEF))
        ELSE
 ...
 1610     FORMAT(3X,'Energy window = (',1P,E12.5,',',E12.5,') eV',
     1      /3X,'Number of energy bins = ',I4,'  (uniform mesh)')
          BDIE(NDIDEF)=1.000000001D0*(EDIU(NDIDEF)-EDIL(NDIDEF))
     1      /DBLE(NDICH(NDIDEF))
        ENDIF
C
...
C
        IF(LDELOG(NDEDEF)) THEN
          ...
          BDEEL(NDEDEF)=1.000000001D0*(EDEUL(NDEDEF)-EDELL(NDEDEF))
     1      /DBLE(NDECH(NDEDEF))
        ELSE
          WRITE(26,1610) EDEL(NDEDEF),EDEU(NDEDEF),NDECH(NDEDEF)
          BDEE(NDEDEF)=1.000000001D0*(EDEU(NDEDEF)-EDEL(NDEDEF))
     1      /DBLE(NDECH(NDEDEF))
        ENDIF
C
   44   CONTINUE
...
C
 ...
 1713   FORMAT(3X,'Numbers of bins:   NBDX =',I4,', NBDY =',I4,
     1    ', NBDZ =',I4)
        DO I=1,3
          BDOSE(I)=1.000000001D0*(DXU(I)-DXL(I))/DBLE(NDB(I))
...
C
     ...
     1             (((PDA2(I,J,K),K=1,NBPH),J=1,NBTH),I=1,3)
        ...
        IF(NDIDEF.GT.0) THEN
          READ (9,*,ERR=90) (RLAST(ID),ID=1,NDIDEF)
          READ (9,*,ERR=90) (RWRITE(ID),ID=1,NDIDEF)
          DO ID=1,NDIDEF
...
C
   ...
   93         CONTINUE
              KODE=0
              CALL RDPSF(IPSFU,NSHI,ISEC,KODE)
              IF(KODE.EQ.-1) THEN
                GO TO 94
              ELSE
                RWR=RWR+1.0D0
...
C
 ...
 1903         FORMAT(/3X,'+ The phase-space file ',A20,
   ...
   95     CONTINUE
          ...
          KODE=0
          CALL RDPSF(IPSFI,NSHI,ISEC,KODE)
...
C
            IF(IPSF(IDET).EQ.1) THEN
              ...
              CALL N2CH10(NSHJ,LC10B,NDIGB)
              WRITE(20+IDET,'(I2,1P,8E13.5,I3,I2,I2,1X,A,1X,A)')
...
C
            ...
            IF(LDILOG(IDET)) THEN
              IE=1.0D0+(LOG(E)-EDILL(IDET))/BDIEL(IDET)
            ELSE
...
C  ----  Energy is locally deposited in the material.
          DEP=(NSPL1-1)*E*WGHT
          DEBO(IBODY)=DEBO(IBODY)+DEP
          IF(LDOSEM) THEN  ! Particle inside the dose enclosure.
            IF((X.GT.DXL(1).AND.X.LT.DXU(1)).AND.
     1         (Y.GT.DXL(2).AND.Y.LT.DXU(2)).AND.
     1         (Z.GT.DXL(3).AND.Z.LT.DXU(3))) THEN
              I1=1.0D0+(X-DXL(1))*BDOSER(1)
...
C  ----  Energy is locally deposited in the material.
      DEBO(IBODY)=DEBO(IBODY)+DEP
      IF(LDOSEM) THEN  ! The particle is inside the dose enclosure.
        IF((X.GT.DXL(1).AND.X.LT.DXU(1)).AND.
     1     (Y.GT.DXL(2).AND.Y.LT.DXU(2)).AND.
     1     (Z.GT.DXL(3).AND.Z.LT.DXU(3))) THEN
          I1=1.0D0+(X-DXL(1))*BDOSER(1)
...
C
        DO KD=1,NDEDEF
          ...
          IF(DEDE(KD).GT.1.0D-5) THEN
            IF(LDELOG(KD)) THEN
              KE=1.0D0+(LOG(DEDE(KD))-EDELL(KD))/BDEEL(KD)
            ELSE
...
C  ****  Transmitted electrons.
      OPEN(9,FILE='energy-el-trans.dat')
      WRITE(9,9110)
 9110 FORMAT(
     1  1X,'#  Results from PENMAIN.',
...
C  ****  Backscattered electrons.
      OPEN(9,FILE='energy-el-back.dat')
      WRITE(9,9120)
 9120 FORMAT(
     1  1X,'#  Results from PENMAIN.',
...
C  ****  Transmitted photons.
      OPEN(9,FILE='energy-ph-trans.dat')
      WRITE(9,9130)
 9130 FORMAT(
     1  1X,'#  Results from PENMAIN.',
...
C  ****  Backscattered photons.
      OPEN(9,FILE='energy-ph-back.dat')
      WRITE(9,9140)
 9140 FORMAT(
     1  1X,'#  Results from PENMAIN.',
...
C  ****  Transmitted positrons.
      OPEN(9,FILE='energy-po-trans.dat')
      WRITE(9,9150)
 9150 FORMAT(
     1  1X,'#  Results from PENMAIN.',
...
C  ****  Backscattered positrons.
      OPEN(9,FILE='energy-po-back.dat')
      WRITE(9,9160)
 9160 FORMAT(
     1  1X,'#  Results from PENMAIN.',
...
C                       SUBROUTINE RDPSF
C  *********************************************************************
      SUBROUTINE RDPSF(IUNIT,NSHI,ISEC,KODE)
C
...
C  with KODE=0. If the psf contains some particles, the oputput value of
C  KODE is set equal to 1. In subsequent calls to subroutine RDPSF, the
C  output value KODE=1 indicates that a valid record has been read and
C  that the state variables of a particle have been loaded in common
...
C  output value KODE=-1 indicates that all particle records in the psf
C  have been read (or that the psf was empty). Note that when KODE=-1,
...
C  ****  Main-PENELOPE common.
      ...
      COMMON/TRACK/E,X,Y,Z,U,V,W,WGHT,KPAR,IBODY,MAT,ILB(5)
C
      COMMON/CPSFRD/E0,X0,Y0,Z0,U0,V0,W0,WGHT0,KPAR0,ILB10,ILB20,ILB30,
     1  ILB40,NSHI0,ILAST
C
      IF(KODE.EQ.0) THEN
        NSHI=0
        ISEC=0
   10   CONTINUE
        READ(IUNIT,'(A200)',END=11,ERR=10) PSFREC
        READ(PSFREC,*,ERR=10,END=10) KPAR0,E0,X0,Y0,Z0,U0,V0,W0,WGHT0,
     1    ILB10,ILB20,ILB30,ILB40,NSHI0
        ILAST=0
        KODE=1
        RETURN
   11   CONTINUE
...
C
   20 CONTINUE
      READ(IUNIT,'(A200)',END=21,ERR=20) PSFREC
      READ(PSFREC,*,ERR=20,END=20) KPAR0,E0,X0,Y0,Z0,U0,V0,W0,WGHT0,
     1  ILB10,ILB20,ILB30,ILB40,NSHI0
      GO TO 22
   21 CONTINUE
      ILAST=1
